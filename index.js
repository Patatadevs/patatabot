...
client.on('messageCreate', async (msg) => {
  ...
  if (msg.content.toLowerCase() === 'patata') {
    ...
    // Si exceden el límite, poner al usuario en timeout
    if (recentTimestamps.length > MESSAGE_LIMIT) {
      try {
        // Poner al usuario en timeout (silencio temporal)
        await msg.member.timeout(MUTE_TIME, 'Exceso de mensajes "patata"')

        // Enviar un mensaje de advertencia al canal
        msg.channel.send(`<@${userId}> has sido muteado por abuso de patata.`)
        
        // Limpiar el contador de mensajes del usuario
        userMessageCounts.delete(userId)
      } catch (error) {
        console.error('Error al poner al usuario en timeout:', error)
      }
    } else {
